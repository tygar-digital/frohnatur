...
Weiche, warme Oberflächen mit semi-transparenten Hintergründen und sanften Schatten.

### Cards

| Klasse | Hintergrund | Einsatz |
|--------|-------------|---------|
| `.soft-card` | `rgba(white, 0.6)` | Floating Card auf hellem Grund |
| `.soft-card-warm` | `rgba(cream, 0.45)` | Card auf warmem Hintergrund |
| `.soft-card-dark` | `rgba(brown-mid, 0.18)` | Card auf dunklem Grund |

Alle Cards: `border-radius: 20px`, subtiler 1px Border (`brown-warm` bei 6–10% Opacity), Schatten `0 4px 20px`.

### Navigation

`.soft-nav` — Glass-Effekt mit `backdrop-filter: blur(20px)`. Weißer Hintergrund bei 92% Opacity, 16px Border-Radius. Einziges Element mit Backdrop-Filter (Phase 4 Review: Glassmorphism runtergeschraubt).

### Buttons

`.soft-btn` — Border-Radius 14px, Schatten `0 4px 16px`. Hover: translateY(-2px) + stärkerer Schatten. Active: scale(0.96).

### Icons

`.soft-icon-circle` — Runder Container (`border-radius: 50%`), Cream-Hintergrund bei 60% Opacity.

### Divider

- `.soft-divider` — Horizontaler 1px Gradient (transparent → brown-warm 12% → transparent)
- `.soft-divider-dark` — Variante für dunkle Hintergründe (cream-warm 18%)

### Schatten

`.soft-shadow-lg` — Großer Schatten für schwebende Bilder: `0 12px 40px rgba(brown-dark, 0.12)`.
...
24 dekorative Kaffeebohnen über die gesamte Seitenhöhe verteilt. Sehr subtil (Opacity 0.05–0.12), verschiedene Rotationen und Größen. `pointer-events: none`, responsive Skalierung via `clamp()`. Farben: brown-warm, brown-mid, brown-dark, cream-warm.

### Paper Grain Texture

`.texture-grain` — SVG-basiertes Rauschen als Pseudo-Element. Sehr zurückhaltend: Opacity 0.025 auf hellem, 0.015 auf dunklem Grund.

### Warm Glow

`.warm-glow` — Radialer Gradient (cream-warm bei 15% Opacity) als atmosphärisches Detail auf hellen Sektionen.

---

## Motion & Animation

Bewegung fühlt sich an wie der erste Schluck Kaffee am Morgen — sanft, warm, wach machend. Nicht hektisch, nicht dramatisch.

### Entrance-Sequenz (Hero)

Gestaffelter Aufbau beim Laden:
1. Navigation (fadeIn, 0.1s Delay)
2. Lampe (Drop-Animation, 0.25s Delay)
3. Headline (fadeInUp, 0.55s Delay)
4. Subtitle (fadeInUp, 0.75s Delay)
5. Illustration (slideUp, 0.65s Delay)
6. Scroll-Indicator (fadeIn, 1.4s Delay)

### Idle-Animationen

- **Lampe:** Sanftes Pendeln (sway, 7s infinite, ±0.7deg)
- **Charakter-Illustration:** Schwebendes Auf-und-Ab (float, 5s infinite, ±7px)
- **Scroll-Indikator:** Pulsierender Scale-Effekt (3s infinite)

### Scroll-Triggered

Alle Sektionen nutzen `IntersectionObserver` (threshold 0.15) für gestaffelte fadeInUp-Animationen beim Einscrolfen. CSS-Variable `--*-delay` steuert die Staffelung pro Element.

### Galerie Film-Strip

Endlos-Animation (translateX) mit 18s (Mobile) / 22s (Desktop). Pausiert bei Pointer-Drag, berechnet Animation-Offset nach Drag-Ende.

### Reduced Motion

Alle CSS-Dateien respektieren `@media (prefers-reduced-motion: reduce)`:
- Animationen deaktiviert
- Opacity sofort auf 1
- Scroll-Behavior auf auto
- Galerie-Track wird scrollbar statt animiert
